"""Explicit immutable compilation session."""
...
class CompilationSession:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.seed, bool) or not isinstance(self.seed, int) or self.seed < 0:
            raise ValueError("compilation seed must be a non-negative integer")
        ...
        if any(not isinstance(feature, str) or not feature for feature in features):
            raise ValueError("compilation features must be non-empty strings")
    ...
    def with_binding(self, binding: BindingValue) -> CompilationSession:
        return replace(self, bindings=self.bindings.with_binding(binding))

    def with_options(self, **changes: Any) -> CompilationSession:
    ...
    def require(self, *axes: BindingAxis) -> None:
        ...
        if BindingAxis.TARGET in axes:
            ...
            if target is not None and not target.satisfies(self.target_requirements):
                raise BindingError(f"target {target.name!r} does not satisfy compilation requirements")
        if BindingAxis.DEPLOYMENT in axes:
            ...
            if deployment is not None and not deployment.satisfies(self.target_requirements):
                raise BindingError(f"deployment {deployment.name!r} does not satisfy compilation requirements")
