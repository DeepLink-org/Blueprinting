"""Closed-world canonical codec used by compiler IR snapshots.
...
def record_type(tag: str) -> Callable[[type[T]], type[T]]:
    """Register a frozen dataclass for canonical round-trip serialization."""
...
def _decode(value: Any) -> Any:
    ...
    if set(value) == {"$type", "fields"}:
        ...
        try:
            decoded = {name: _decode(item) for name, item in payload.items()}
