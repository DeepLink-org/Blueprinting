"""Typed decoder-only Transformer frontend.

The frontend deliberately imports *semantic* model and execution facts.  It
does not attach a target, a kernel choice, or an estimated duration.  The
coarse ``transformer.decoder_training`` operation is decomposed by later
passes, which makes the operation-counting rules inspectable at an IR
boundary instead of hiding them in an end-to-end formula.
"""
...
class TransformerExecutionSpec:
    """Compile-time training strategy used by the comparison experiment.
...
def build_transformer_model_ir(model: TransformerModelSpec, *, datatype: str = "float16") -> ModelIR:
    ...
    if datatype not in {"float16", "bfloat16", "float32", "float8"}:
