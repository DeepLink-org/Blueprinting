...
therefore measures agreement; it cannot make the compiled result agree by
construction.
"""
...
class VidurComponentComparison:
    ...
    def absolute_error_seconds(self) -> float | None:
        if self.baseline_seconds is None:
            return None
        return self.compiled_seconds - self.baseline_seconds

    @property
    def relative_error_percent(self) -> float | None:
        if self.baseline_seconds is None or self.baseline_seconds == 0:
            return None
        return (self.compiled_seconds - self.baseline_seconds) / self.baseline_seconds * 100
...
class VidurPhaseComparison:
    ...
    @property
    def compiled_comparable_block_seconds(self) -> float:
        return sum(component.compiled_seconds for component in self.matched_components)
    ...
    @property
    def excluded_compiled_block_seconds(self) -> float:
        return self.compiled_block_seconds - self.compiled_comparable_block_seconds
    ...
    def comparable_subtotal_relative_error_percent(self) -> float | None:
        ...
        if reference == 0:
            return None
        return (self.compiled_comparable_block_seconds - reference) / reference * 100
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "compiled_block_seconds": self.compiled_block_seconds,
            "compiled_comparable_block_seconds": self.compiled_comparable_block_seconds,
            "baseline_comparable_block_seconds": self.baseline_comparable_block_seconds,
            "excluded_compiled_block_seconds": self.excluded_compiled_block_seconds,
...
class VidurExperimentCase:
    """One static phase point compiled by Blueprinting before comparison."""
...
def compare_inference_phase_to_vidur(
    ...
    for plan_task, task_estimate in zip(plan.tasks, estimate.tasks):
        ...
        components.append(
            VidurComponentComparison(
                ...
                compiled_seconds=task_estimate.total_seconds,
    ...
    return VidurPhaseComparison(
        ...
        compiled_block_seconds=estimate.block_seconds,
...
def run_vidur_experiment(
    ...
    """Compile each phase independently, then compare both analytical modes."""
    ...
    return VidurExperimentReport(
        schema="blueprinting.vidur-baseline-experiment.v1",
        baseline_revision=baseline.revision,
        policy={
