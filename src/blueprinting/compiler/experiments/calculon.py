...
* an oracle for independently checking compiled work and schedule metrics;
...
class MetricComparison:
    ...
    def relative_error_percent(self) -> float:
        if self.reference == 0:
            return 0.0 if self.compiled == 0 else float("inf")
        return (self.compiled - self.reference) / self.reference * 100
...
class CalculonCaseReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "memory_bytes": {
                "compiled": self.calibrated.memory.total,
            ...
            "recompute_counter_audit": {
                "compiled_explicit_operations": _phase_operations_from_estimate(
...
def _run_calculon(
    ...
    logger = logging.getLogger("blueprinting.compiler.experiments.calculon")
...
def run_calculon_experiment(cases: tuple[CalculonCase, ...]) -> CalculonExperimentReport:
    ...
    for case in cases:
        ...
        plan, checkpoints, model_digest, distributed_digest = _compile_plan(model, execution)
    ...
    return CalculonExperimentReport(
        schema="blueprinting.calculon-calibration-experiment.v1",
