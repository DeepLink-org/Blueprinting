...
class DistributeTransformerInferencePass(CompilerPass[ModelIR, DistributedTaskIR]):
    ...
    def run(self, ir: ModelIR, context: PassContext) -> DistributedTaskIR:
        ...
        invocations, block_memory = compile_transformer_inference_block(
            model,
            execution,
            phase=phase,
...
class PlanTransformerInferencePass(CompilerPass[DistributedTaskIR, PortablePlanIR]):
