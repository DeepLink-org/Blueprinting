...
@record_type("compiler.concrete.device")
...
@enum_type("compiler.concrete.queue_kind")
...
@record_type("compiler.concrete.queue")
@dataclass(frozen=True)
class QueueSpec:
    id: QueueId
    device: DeviceId
...
@record_type("compiler.concrete.memory_region")
@dataclass(frozen=True)
class MemoryRegion:
    id: MemoryRegionId
    device: DeviceId
...
@record_type("compiler.concrete.buffer_binding")
@dataclass(frozen=True)
class BufferBinding:
    id: BufferId
...
@record_type("compiler.concrete.implementation_ref")
@dataclass(frozen=True)
...
@enum_type("compiler.concrete.access_mode")
...
@record_type("compiler.concrete.buffer_use")
@dataclass(frozen=True)
class BufferUse:
    buffer: BufferId
    access: AccessMode
...
@enum_type("compiler.concrete.command_kind")
...
@record_type("compiler.concrete.command")
@dataclass(frozen=True)
class ConcreteCommand:
    ...
    lineage: Lineage
    wait_tokens: tuple[TokenId, ...] = ()
    signal_tokens: tuple[TokenId, ...] = ()
    attributes: FrozenDict = field(default_factory=FrozenDict)

    def __post_init__(self) -> None:
        require_instance(self.id, CommandId, "command ID")
        require_instance(self.kind, CommandKind, "command kind")
        require_instance(self.lineage, Lineage, "command lineage")
        if self.queue is not None:
            require_instance(self.queue, QueueId, "command queue")
        if self.implementation is not None:
            require_instance(self.implementation, ImplementationRef, "command implementation")
        object.__setattr__(
            self,
            "dependencies",
        ...
        object.__setattr__(self, "wait_tokens", typed_tuple(self.wait_tokens, TokenId, "command wait tokens"))
        object.__setattr__(
            self,
            "signal_tokens",
            typed_tuple(self.signal_tokens, TokenId, "command signal tokens"),
        )
        object.__setattr__(self, "attributes", frozen_map(self.attributes))
...
@record_type("compiler.ir.concrete_plan.v1")
@dataclass(frozen=True)
class ConcretePlanIR(CanonicalIRMixin):
    ...
    def verify(self) -> VerificationReport:
        ...
        for index, buffer in enumerate(self.buffers):
            ...
            region = region_by_id.get(buffer.memory_region)
            if region is None:
                bag.error("reference.unknown", f"unknown memory region {buffer.memory_region}", *path, "memory_region")
            else:
                if buffer.offset_bytes % max(buffer.alignment_bytes, region.alignment_bytes) != 0:
                    bag.error(
                        "buffer.alignment", "buffer offset violates buffer or region alignment", *path, "offset_bytes"
                    )
                if buffer.offset_bytes + buffer.size_bytes > region.capacity_bytes:
        ...
        for index, command in enumerate(self.commands):
            ...
            if command.kind in queue_kinds and command.queue is None:
            ...
            if command.kind is CommandKind.LAUNCH and command.queue in queue_by_id:
                device = queue_by_id[command.queue].device
                for buffer_id in used_buffers:
                    binding = buffer_by_id.get(buffer_id)
                    region = region_by_id.get(binding.memory_region) if binding is not None else None
                    if region is not None and region.device != device:
                        bag.error(
