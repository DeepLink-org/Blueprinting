...
class PrintSchedulePass(Pass):
    ...
    def _format(self, ir: ScheduleIR) -> str:
        ...
        lines.append(
            f"  stages={len(ir.stages)}, devices={ir.num_devices}, ops={ir.total_ops}"
        )
        lines.append(f"{'='*100}")
        ...
        for stage_id, stage in sorted(ir.stages.items()):
            for device_id, device in sorted(stage.devices.items()):
                ...
                # 表头
                lines.append(
                    f"  {'seq':>4}  {'Op类型':<12}  {'时间':>10}  {'耗时':>10}  {'stream':<8}  {'来源Block'}"
                )
                lines.append(
                    f"  {'-'*4}  {'-'*12}  {'-'*10}  {'-'*10}  {'-'*8}  {'-'*40}"
                )
...
class PrintTimelinePass(Pass):
    ...
    def _format(self, ir: TimelineIR) -> str:
        ...
        # 表头
        lines.append(
            f"  {'时间':>12}  {'事件类型':<14}  {'Op名称':<15}  {'dev':>3}  {'stream':<8}  {'来源Block'}"
        )
