...
from ..types import EventType, StreamType, TimelineIR
    ...
    def run(self, ir: TimelineIR) -> TimelineIR:
        ...
        ir.metadata["overlap_analysis"] = {
            ...
            "total": {
                ...
                "effective_time": total_compute
                + total_comm
                - total_overlap * self.overlap_efficiency,
    ...
    def _get_intervals(
        ...
        for event in ir.events:
            ...
            if event.event_type in (EventType.COMPUTE_START, EventType.COMM_START):
                ...
                event.event_type in (EventType.COMPUTE_END, EventType.COMM_END)
                and resource in starts
            ):
