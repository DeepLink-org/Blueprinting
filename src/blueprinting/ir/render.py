...
def _format_bytes(val: int | float | None) -> str:
    """格式化字节数为人类可读格式."""
    if val is None:
        return "-"
...
def _format_time(val: int | float | None) -> str:
    """格式化时间为人类可读格式."""
    if val is None:
        return "-"
    if not isinstance(val, (int, float)):
...
class GraphRenderMixin:
    ...
    def to_terminal(self, max_depth: int = 4, max_children: int = 10) -> str:
        ...
        def _add_block_to_tree(parent_tree: Tree, block, depth: int = 0):
            ...
            for i, child in enumerate(children):
                if i >= max_children:
                    block_branch.add(
                        f"[dim]... ({len(children) - max_children} more)[/dim]"
                    )
                    break
        ...
        for bt, stats in sorted(block_stats.items()):
            table.add_row(
                bt,
                str(stats["count"]),
                f"{stats['with_params']}" if stats["with_params"] else "-",
            )
    ...
    def to_tree_html(self, max_children: int | None = 10) -> str:
        ...
        def _render_block(block, depth: int = 0) -> str:
            ...
            if shard:
                badges.append(
                    f'<span style="color:#f59e0b;margin-left:4px;">[{html.escape(shard)}]</span>'
                )
...
class ScheduleRenderMixin:
    ...
    def to_terminal(self, max_ops: int = 10) -> str:
        ...
        if not HAS_RICH:
            return (
                f"ScheduleIR(stages={len(schedule.stages)}, ops={schedule.total_ops})"
            )
        ...
        for stage_id, stage in sorted(schedule.stages.items()):
            ...
            stage_branch = tree.add(
                f"[cyan]Stage {stage_id}[/cyan] [dim](ops={stage_ops})[/dim]"
            )

            for device_id, device in sorted(stage.devices.items()):
                dev_branch = stage_branch.add(
                    f"[yellow]Device {device_id}[/yellow] [dim](ops={len(device.ops)})[/dim]"
                )

                for op in device.ops[:3]:
                    phase_color = {"fw": "green", "bw": "red", "opt": "magenta"}.get(
                        op.phase, "white"
                    )
                    dur_str = (
                        _format_time(op.duration)
                        if isinstance(op.duration, (int, float))
                        else str(op.duration)
                    )
                    dev_branch.add(
                        f"[{phase_color}]{op.op_type}[/{phase_color}] '{op.name}' [dim]seq={op.event_seq}, dur={dur_str}[/dim]"
                    )
        ...
        for op in list(schedule.iter_ops())[:max_ops]:
            phase_style = {"fw": "green", "bw": "red", "opt": "magenta"}.get(
                op.phase, "white"
            )
            table.add_row(
                str(op.event_seq),
    ...
    def to_tree_html(self, max_ops: int = 5) -> str:
        ...
        for stage_id, stage in sorted(schedule.stages.items()):
            stage_ops = sum(len(d.ops) for d in stage.devices.values())
            lines.append(
                f"<li>Stage {stage_id} <span style='color:#6b7280'>(ops={stage_ops})</span><ul style='margin:0; padding-left:16px;'>"
            )

            for device_id, device in sorted(stage.devices.items()):
                dev_label = f"Device {device_id} <span style='color:#6b7280'>(ops={len(device.ops)})</span>"
                preview_parts = [
                    f"{html.escape(op.op_type or '')}({html.escape(op.name or '')}) <span style='color:#6b7280'>(seq={op.event_seq})</span>"
                ...
                for op in device.ops[:max_ops]:
                    lines.append(
                        f"<li>{html.escape(op.op_type or '')}({html.escape(op.name or '')}) <span style='color:#6b7280'>(seq={op.event_seq})</span></li>"
                    )

                if len(device.ops) > max_ops:
                    lines.append(
                        f"<li><details><summary><span style='color:#6b7280'>... ({len(device.ops) - max_ops} more)</span></summary><ul style='margin:0; padding-left:16px;'>"
                    )
                    for op in device.ops[max_ops:]:
                        lines.append(
                            f"<li>{html.escape(op.op_type or '')}({html.escape(op.name or '')}) <span style='color:#6b7280'>(seq={op.event_seq})</span></li>"
...
class TimelineRenderMixin:
    ...
    def to_terminal(self, max_events: int = 15) -> str:
        ...
        peak = (
            max(s.peak for s in timeline.memory_snapshots)
            if timeline.memory_snapshots
            else 0
        )
        ...
        all_devices = {e.device for e in events}
        ...
        stats_text.append(
            f"{_format_time(makespan) if isinstance(makespan, (int, float)) else str(makespan)}\n",
            style="green",
        )
        ...
        summary = Panel(
            stats_text,
            title="[bold blue]TimelineIR[/bold blue]",
            subtitle=f"Events: {len(events)} | Devices: {num_devices}",
            border_style="blue",
        )
        console.print(summary)
        ...
        # 事件表格
        table = Table(
            title=f"事件列表 (前{max_events})", box=box.ROUNDED, show_header=True
        )
        table.add_column("Seq", justify="right", style="dim")
        ...
        for seq, event in enumerate(events[:max_events]):
            ...
            table.add_row(
                ...
                event.op_type or "-",
                (
                    _format_bytes(event.size)
                    if isinstance(event.size, (int, float))
                    else str(event.size or "-")
                ),
            )
    ...
    def to_tree_html(self, max_events: int = 5) -> str:
        ...
        for device_id in sorted(device_stream_events.keys()):
            ...
            lines.append(
                f"<li><details><summary style='cursor:pointer;'>Device {device_id} <span style='color:#6b7280;'>(events={device_total})</span></summary><ul style='margin:0; padding-left:16px;'>"
            )
            ...
            for stream_name in sorted(streams.keys()):
                ...
                lines.append(
                    f"<li><details><summary style='cursor:pointer;'><span style='color:{color};font-weight:500;'>{stream_name}</span> <span style='color:#6b7280;'>({len(stream_events)} events)</span></summary><ul style='margin:0; padding-left:16px;'>"
                )
                ...
                for event in stream_events[:max_events]:
                    ...
                    time_str = (
                        f"{event.time*1e3:.3f}ms"
                        if isinstance(event.time, (int, float))
                        else str(event.time)
                    )
                    lines.append(
                        f"<li><span style='color:#9ca3af;'>[{time_str}]</span> <span style='color:{color};'>{event_type}</span> {op_type}</li>"
                    )

                if len(stream_events) > max_events:
                    lines.append(
                        f"<li><details><summary style='color:#9ca3af;cursor:pointer;'>... ({len(stream_events) - max_events} more)</summary><ul style='margin:0; padding-left:16px;'>"
                    )
                    for event in stream_events[max_events:]:
                        event_type = html.escape(event.event_type.value)
                        op_type = html.escape(event.op_type or "-")
    ...
    def to_table(
        self, max_rows: int = 100, device: int | None = None
    ) -> pd.DataFrame:
        """渲染为 DataFrame 表格."""
        if not HAS_PANDAS:
            raise ImportError("pandas is required for table rendering")
...
class SimulationResultRenderMixin:
    ...
    def to_terminal(self) -> str:
        ...
        console.print(
            Panel(
                main_text, title="[bold blue]仿真结果[/bold blue]", border_style="blue"
            )
        )
        ...
        if result.time_breakdown:
            ...
            for name, val, color in [
                ...
                if val:
                    pct = val / total * 100 if total else 0
                    time_table.add_row(
                        f"[{color}]{name}[/{color}]", _format_time(val), f"{pct:.1f}%"
                    )
            time_table.add_row(
                "[bold]合计[/bold]",
                f"[bold]{_format_time(total)}[/bold]",
                "[bold]100%[/bold]",
            )
            console.print()
