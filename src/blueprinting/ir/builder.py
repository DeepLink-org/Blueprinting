...
from .graph import GraphIR, ModuleNode, BlockNode, OpNode, TensorRef
...
class IRBuilder:
    ...
    def _register_output_tensor(self, name: str, shape: List = None, 
                                 dtype: str = "float16", producer: str = None):
    ...
    def add_op(self, name: str, op_type: str, inputs: List[str] = None,
               output_name: str = None, **attrs) -> OpNode:
    ...
    def linear(self, name: str, input_tensor: str, in_features: Union[int, Expr],
               out_features: Union[int, Expr], bias: bool = True,
               shard: str = None, **attrs) -> OpNode:
        """Add a Linear layer.
    ...
    def rmsnorm(self, name: str, input_tensor: str, 
                normalized_shape: Union[int, Expr], **attrs) -> OpNode:
    ...
    def layernorm(self, name: str, input_tensor: str,
                  normalized_shape: Union[int, Expr], **attrs) -> OpNode:
    ...
    def attention(self, name: str, inputs: List[str], num_heads: Union[int, Expr],
                  head_dim: Union[int, Expr], seq_len: Union[int, Expr] = None,
                  batch_size: Union[int, Expr] = None, **attrs) -> OpNode:
    ...
    def activation(self, name: str, input_tensor: str, 
                   activation_type: str = "SiLU", **attrs) -> OpNode:
    ...
    def elementwise(self, name: str, op_type: str, inputs: List[str], **attrs) -> OpNode:
    ...
    def comm(self, name: str, comm_type: str, input_tensor: str,
             num_peers: int = 1, **attrs) -> OpNode:
    ...
    def allreduce(self, name: str, input_tensor: str, num_peers: int = 1, **attrs) -> OpNode:
        """Add an AllReduce operation."""
        return self.comm(name, "AllReduce", input_tensor, num_peers, **attrs)
    
    def allgather(self, name: str, input_tensor: str, num_peers: int = 1, **attrs) -> OpNode:
        """Add an AllGather operation."""
        return self.comm(name, "AllGather", input_tensor, num_peers, **attrs)
    
    def reduce_scatter(self, name: str, input_tensor: str, num_peers: int = 1, **attrs) -> OpNode:
...
def build_transformer_layer(
    ...
    with builder.block(f"layer{layer_idx}", "TransformerLayer", layer_idx=layer_idx):
        ...
        with builder.block("attn", "Attention"):
            ...
            q = builder.linear("q_proj", norm_out, hidden, hidden, shard=shard, batch_seq=batch_seq)
            k = builder.linear("k_proj", norm_out, hidden, hidden, shard=shard, batch_seq=batch_seq)
            v = builder.linear("v_proj", norm_out, hidden, hidden, shard=shard, batch_seq=batch_seq)
            
            # Attention computation
            attn = builder.attention(
                "mha",
                [f"{q.name}_out", f"{k.name}_out", f"{v.name}_out"],
                num_heads, head_dim, seq_len, batch_size,
                shard="tp_col" if tp > 1 else None,
            )
            
            # Output projection
            out_proj = builder.linear(
                "out_proj", f"{attn.name}_out", hidden, hidden,
                shard="tp_row" if tp > 1 else None,
                batch_seq=batch_seq,
            )
            
            # Residual
            res1 = builder.add("residual", [input_tensor, f"{out_proj.name}_out"],
        ...
        with builder.block("ffn", "FFN"):
            ...
            shard = "tp_col" if tp > 1 else None
            fc1 = builder.linear("fc1", norm2_out, hidden, feedforward, shard=shard, batch_seq=batch_seq)
            act = builder.activation("act", f"{fc1.name}_out", "SiLU",
                                     num_elements=batch_seq * feedforward,
                                     shard="tp_col" if tp > 1 else None)
            fc2 = builder.linear(
                "fc2", f"{act.name}_out", feedforward, hidden,
                shard="tp_row" if tp > 1 else None,
                batch_seq=batch_seq,
            )
            
            # Residual
            res2 = builder.add("residual", [attn_out, f"{fc2.name}_out"],
                               num_elements=batch_seq * hidden)
...
def build_transformer_model(
    ...
    for i in range(num_layers):
        current_tensor = build_transformer_layer(
            builder, i, hidden, feedforward, num_heads, head_dim,
            seq_len, batch_size, current_tensor, tp
        )
    
    # Final norm
    with builder.block("final", "Output"):
        final_norm = builder.rmsnorm("norm", current_tensor, hidden,
                                     batch_seq=batch_size * seq_len)
