...
class ScheduleIR:
    ...
    def add_op(
        self, op: ScheduledOp, stage_id: int = 0, device_id: int = 0
    ) -> ScheduledOp:
        ...
        for op in self.get_ops_on_device(device):
            ...
            for tensor in op.tensors_alloc:
                if tensor.free_time is not None:
                    events.append(
                        (tensor.free_time, -tensor.size, f"-{tensor.tensor_id}")
                    )
        ...
        if all_numeric:
            ...
            for _, delta, tag in events:
                if tag.startswith("+"):
                    total_alloc = (
                        total_alloc + delta
                        if isinstance(delta, Expr)
                        else total_alloc + delta
                    )
            return total_alloc
    ...
    def summary(self) -> str:
        ...
        for stage in self.iter_stages():
            ...
            lines.append(
                f"│   stage{stage.stage_id}: {dev_count} devices, {op_count} ops"
            )
    ...
    def tree(self, max_ops: int = 5) -> str:
        ...
        for stage in self.iter_stages():
            ...
            for i, device in enumerate(devices):
                ...
                lines.append(
                    f"{dev_prefix} ◇ Device {device.device_id} ({len(device.ops)} ops)"
                )
                ...
                for j, op in enumerate(ops_to_show):
                    is_last_op = (
                        j == len(ops_to_show) - 1 and len(device.ops) <= max_ops
                    )
                    marker = "└─" if is_last_op else "├─"
                    lines.append(
                        f"{op_prefix}{marker} ◆ {op.op_type}({op.name}) seq={op.event_seq}"
                    )

                if len(device.ops) > max_ops:
                    lines.append(
                        f"{op_prefix}└─ ... ({len(device.ops) - max_ops} more)"
                    )
