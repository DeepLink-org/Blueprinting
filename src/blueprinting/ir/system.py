...
class ProcessorConfig:
            ...
            for dtype, dtype_cfg in cfg["matrix"].items():
                self._datatypes[f"matrix_{dtype}"] = {
                    ...
                    "efficiency": EfficiencyCurve(
                        [
                            (gflops * 1e9, eff)
                            for gflops, eff in dtype_cfg.get(
    ...
    def get_throughput(
        self, flops: float, engine: str = "matrix", dtype: str = "float16"
    ) -> float:
...
class MemoryConfig:
    ...
    def __init__(self, cfg: Dict):
        ...
        self._efficiency = EfficiencyCurve(
            [(mb * 1e6, eff) for mb, eff in mb_efficiency], unit_scale=1e6
        )
...
class NetworkConfig:
    ...
    def time(self, op_type: str, comm_bytes: float, num_peers: int) -> float:
        ...
        if (
            op_cfg is not None
            and isinstance(op_cfg, (list, tuple))
            and len(op_cfg) == 2
        ):
...
class SystemConfig:
    ...
    def __init__(self, config: Union[str, Path, Dict], datatype: str = "float16"):
    ...
    def compute_op_time(
        self, flops: float, memory_bytes: float, engine: str = "matrix"
