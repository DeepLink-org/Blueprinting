...
class MemorySnapshot:
    ...
    def __repr__(self) -> str:
        ...
        return f"MemorySnapshot(t={self.time}, dev={self.device}, alloc={self.allocated})"
...
class TimelineIR:
    ...
    def peak_memory(self, device: int = 0) -> Union[int, Expr]:
        ...
        memory_events = [
            ...
            if e.device == device and e.is_memory_event()
        ]
    ...
    def makespan(self, device: Optional[int] = None) -> Union[float, Expr]:
        ...
        events = self.events if device is None else [
