...
from blueprinting.synthesizer.ir import DistributedTaskIR, ModelIR
from blueprinting.synthesizer.ir.common import make_header
...
def test_observer_failure_prevents_analysis_commit(model_ir: ModelIR) -> None:
    ...
    with pytest.raises(PassExecutionError, match="RejectObserver"):
        manager.run(PassPipeline.of(derivation_pass), model_ir, session=session)
...
def test_missing_analysis_fails_before_pass_runs(model_ir: ModelIR) -> None:
    ...
    with pytest.raises(MissingAnalysisError, match="requires missing analyses"):
        PassManager().run(PassPipeline.of(derivation_pass), model_ir, session=SynthesisSession())
