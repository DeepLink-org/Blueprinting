...
实现将 typed derivation context 命名为 `SynthesisSession`。它标识 workload、strategy、target requirement、deployment、calibration revision、feature set 与 deterministic seed；session fingerprint 参与每个 analysis address。这个术语属于 `blueprinting.synthesizer` 实现边界，不代表概念架构中存在一个 Compiler 组件。
